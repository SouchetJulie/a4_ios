...
class MemeViewController: UIViewController {
    ...
    func fetchImage() {
        if let dataModel = self.data {
            // Récupère un type d'image aléatoire
            let index: Int = Int.random(in: 1..<options.count)
            
            // Construit l'URL depuis le modèle
            let string = "https://apimeme.com/meme?meme=\(options[index])&top=\(getFirstWords(text: dataModel.title))&bottom=\(getFirstWords(text: dataModel.body))"
...
func getFirstWords(text: String, number: Int = 3) -> String {
    let parts = text.components(separatedBy: " ")
    let firsts = parts[0..<(min(number, parts.count))]
